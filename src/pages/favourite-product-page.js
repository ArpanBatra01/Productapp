...
function FavouriteProduct() {
  ...
  useEffect(() => {
    let storedfavourite = JSON.parse(localStorage.getItem("favouriteItems")); 
    if (storedfavourite?.length) {
      
      let favouriteProducts = [];

   
      storedfavourite?.map((favouriteItems) => {
        let product = products?.find((elem) => elem.id == favouriteItems);
        if (product) {
